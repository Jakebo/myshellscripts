#!/bin/bash
#===============================================================================
#          FILE:  ver_report_diff_tags.sh
#         USAGE:  ./ver_report_diff_tags.sh
#   DESCRIPTION:  
#       OPTIONS:  ---
#  REQUIREMENTS:  ---
#          BUGS:  ---
...
function usage_help()
{
    ...
    echo "usage     : $0 start_tags end_tags"
    echo "start_tags: tags started"
    echo "end_tags  : tags ended"
    echo "example   : $0 V201.01.00.0013 V201.01.00.0014"
    echo "note      : the date of start_tags must be earlier than the date of end_tags"
    return 1
}
root_path=$PWD
...
tag_old=$1
tag_new=$2
echo Tag diff report: old=${tag_old},new=${tag_new}
...
do
    #echo prj: "${prj}"
    cd ${prj}
    time_from=`git show -1 --date=short --shortstat ${tag_old} | sed -n '3p' | awk '{print $2}'`
    time_to=`git show  -1 --date=short --shortstat ${tag_new} | sed -n '3p' | awk '{print $2}'`
